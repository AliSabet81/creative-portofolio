      ...
      <ResponsiveComponent>
        {({ size }) => {
          return size && size >= 480 ? (
            ...
            <>
              <div className="w-full px-2.5 xs:p-0 flex-col items-start space-y-4 xs:w-max flex xs:items-center justify-center relative group">
                {BtnList.slice(0, BtnList.length / 2).map((btn) => {
                  return <NavButton key={btn.label} x={0} y={0} {...btn} />;
                })}
              </div>
              <div className="w-full px-2.5 xs:p-0 flex-col items-end space-y-4 xs:w-max flex xs:items-center justify-center relative group">
